...
static int __init le_rkm_init_debugfs(void)
{
	...
	entry = proc_create("lk_mesg", 0444, NULL, &rkm_debug_lk_log_fops);
	if (!entry) {
		pr_err("%s:init lk_mesg proc fail\n",__func__);
		return -ENOMEM;
	}
	entry = proc_create("last_kmsg", 0444, NULL, &rkm_debug_kernel_log_fops);
	if (!entry) {
		pr_err("%s:init last_kmsg proc fail\n",__func__);
		return -ENOMEM;
	}
	return 0;
}
