...
static int msm8x16_quin_mi2s_clk_int_codec_mux(void)
{
	...
	val = ioread32(vaddr);
	val = val | 0x00000001;
	pr_err("%s: quin mux configuration = %x\n", __func__, val);
...
{
	int ret = 0;
	pr_err("zhouwl >>>%s, enable = %d, quin_mi2s_clk_ref = %d\n",
			 __func__, enable, atomic_read(&quin_mi2s_clk_ref));
	if(enable) {
			pr_err("enter>>>%s, enable = %d, quat_mi2s_clk_ref = %d\n", __func__, enable, atomic_read(&quin_mi2s_clk_ref));
			...
			pr_err("%s: mi2s_rx_clk enable is %d\n",__func__,mi2s_rx_clk.enable);
			...
			pr_err("enter>>>%s, enable = %d, quin_mi2s_clk_ref = %d\n", __func__, enable, atomic_read(&quin_mi2s_clk_ref));
...
static int msm_quin_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	struct msm8916_asoc_mach_data *pdata =
			snd_soc_card_get_drvdata(card);
	int ret = 0, val = 0;
	pr_err("%s enter: substream = %s  stream = %d, ext_pa = %d,  quin_mi2s_clk_ref= %d\n", __func__,
