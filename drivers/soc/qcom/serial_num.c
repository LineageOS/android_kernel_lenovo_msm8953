...
#define provision_readl(drvdata, off)	__raw_readl(drvdata->provison_base + off)
...
static uint32_t provision_state = 0;

struct sn_drvdata {
	void __iomem		*base;
	void __iomem		*fuse_base;
	void __iomem		*provison_base;
	struct device		*dev;
};
...
static int provison_read(struct seq_file *m, void *v)
{
	struct sn_drvdata *drvdata = sndrvdata;
	...
	if (provision_state == 0)
		provision_state = provision_readl(drvdata, SERIAL_NUM);

	dev_dbg(drvdata->dev, "rpmb provision state: %x\n", provision_state);
...
static int provision_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, provison_read, NULL);
}

static const struct file_operations provision_fops = {
	.open		= provision_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
static void sn_create_proc(void)
{
	...
	entry = proc_create("provision_state", 0 /* default mode */,
			NULL /* parent dir */, &provision_fops);
}

static int sn_fuse_provision_probe(struct platform_device *pdev)
{
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "provision-state");
	if (!res)
		return -ENODEV;
	
	drvdata->provison_base= devm_ioremap(dev, res->start, resource_size(res));
	if (!drvdata->provison_base)
		return -ENOMEM;
...
static int sn_fuse_provision_remove(struct platform_device *pdev)
{
	return 0;
}

static struct of_device_id sn_fuse_provision_match[] = {
	{.compatible = "qcom,sn-fuse"},
	{}
};

static struct platform_driver sn_fuse_provision_driver = {
	.probe          = sn_fuse_provision_probe,
	.remove         = sn_fuse_provision_remove,
	.driver         = {
		.name   = "msm-sn-fuse-provision",
		.owner	= THIS_MODULE,
		.of_match_table = sn_fuse_provision_match,
	},
};

static int __init sn_fuse_provision_init(void)
{
	return platform_driver_register(&sn_fuse_provision_driver);
}
arch_initcall(sn_fuse_provision_init);

static void __exit sn_fuse_provision_exit(void)
{
	platform_driver_unregister(&sn_fuse_provision_driver);
}
module_exit(sn_fuse_provision_exit);
