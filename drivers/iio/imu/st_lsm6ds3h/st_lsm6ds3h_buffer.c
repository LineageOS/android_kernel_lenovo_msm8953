...
#define MIN_BNZ(a, b, c, d)			(((a) < (b)) ? ((a == 0) ? \
						(d) : (c)) : ((b == 0) ? \
						(c) : (d)))
...
static void st_lsm6ds3h_parse_fifo_data(struct lsm6ds3h_data *cdata, u16 read_len, int64_t time_top, u16 num_pattern)
{
	int err;
	u16 fifo_offset = 0;
			...
			if (gyro_sip > 0) {
				if (cdata->fifo_output[ST_MASK_ID_GYRO].timestamp == 0) {
					if (cdata->slower_id == ST_MASK_ID_GYRO)
						cdata->fifo_output[ST_MASK_ID_GYRO].timestamp = time_top - (num_pattern * gyro_sip * cdata->fifo_output[ST_MASK_ID_GYRO].deltatime) - 300000;
					else
						cdata->fifo_output[ST_MASK_ID_GYRO].timestamp = time_top - (num_pattern * gyro_sip * cdata->fifo_output[ST_MASK_ID_GYRO].deltatime) - 300000 -
			...
			if (accel_sip > 0) {
				if (cdata->fifo_output[ST_MASK_ID_ACCEL].timestamp == 0) {
					if (cdata->slower_id == ST_MASK_ID_ACCEL)
						cdata->fifo_output[ST_MASK_ID_ACCEL].timestamp = time_top - (num_pattern * accel_sip * cdata->fifo_output[ST_MASK_ID_ACCEL].deltatime) - 300000;
					else
						cdata->fifo_output[ST_MASK_ID_ACCEL].timestamp = time_top - (num_pattern * accel_sip * cdata->fifo_output[ST_MASK_ID_ACCEL].deltatime) - 300000 -
			...
			if (ext0_sip > 0) {
				if (cdata->fifo_output[ST_MASK_ID_EXT0].timestamp == 0) {
					if (cdata->slower_id == ST_MASK_ID_EXT0)
						cdata->fifo_output[ST_MASK_ID_EXT0].timestamp = time_top - (num_pattern * ext0_sip * cdata->fifo_output[ST_MASK_ID_EXT0].deltatime) - 300000;
					else
						cdata->fifo_output[ST_MASK_ID_EXT0].timestamp = time_top - (num_pattern * ext0_sip * cdata->fifo_output[ST_MASK_ID_EXT0].deltatime) - 300000 -
...
int st_lsm6ds3h_read_fifo(struct lsm6ds3h_data *cdata, bool async)
{
	...
	cdata->slower_id = MIN_BNZ(cdata->fifo_output[ST_MASK_ID_GYRO].sip, cdata->fifo_output[ST_MASK_ID_ACCEL].sip, ST_MASK_ID_GYRO, ST_MASK_ID_ACCEL);
#ifdef CONFIG_ST_LSM6DS3H_IIO_MASTER_SUPPORT
	cdata->slower_id = MIN_BNZ(cdata->fifo_output[cdata->slower_id].sip, cdata->fifo_output[ST_MASK_ID_EXT0].sip, cdata->slower_id, ST_MASK_ID_EXT0);
#endif /* CONFIG_ST_LSM6DS3H_IIO_LIMIT_FIFO */
	...
	if (temp_counter > 0) {
		...
		case ST_MASK_ID_ACCEL:
			if (cdata->fifo_output[ST_MASK_ID_GYRO].sip != 0)
				cdata->fifo_output[ST_MASK_ID_GYRO].deltatime = div64_s64(slower_deltatime * cdata->fifo_output[ST_MASK_ID_ACCEL].sip, cdata->fifo_output[ST_MASK_ID_GYRO].sip);

			if (cdata->fifo_output[ST_MASK_ID_EXT0].sip != 0)
				cdata->fifo_output[ST_MASK_ID_EXT0].deltatime = div64_s64(slower_deltatime * cdata->fifo_output[ST_MASK_ID_ACCEL].sip, cdata->fifo_output[ST_MASK_ID_EXT0].sip);
		...
		case ST_MASK_ID_GYRO:
			if (cdata->fifo_output[ST_MASK_ID_ACCEL].sip != 0)
				cdata->fifo_output[ST_MASK_ID_ACCEL].deltatime = div64_s64(slower_deltatime * cdata->fifo_output[ST_MASK_ID_GYRO].sip, cdata->fifo_output[ST_MASK_ID_ACCEL].sip);

			if (cdata->fifo_output[ST_MASK_ID_EXT0].sip != 0)
				cdata->fifo_output[ST_MASK_ID_EXT0].deltatime = div64_s64(slower_deltatime * cdata->fifo_output[ST_MASK_ID_GYRO].sip, cdata->fifo_output[ST_MASK_ID_EXT0].sip);
		...
		case ST_MASK_ID_EXT0:
			if (cdata->fifo_output[ST_MASK_ID_ACCEL].sip != 0)
				cdata->fifo_output[ST_MASK_ID_ACCEL].deltatime = div64_s64(slower_deltatime * cdata->fifo_output[ST_MASK_ID_EXT0].sip, cdata->fifo_output[ST_MASK_ID_ACCEL].sip);

			if (cdata->fifo_output[ST_MASK_ID_GYRO].sip != 0)
				cdata->fifo_output[ST_MASK_ID_GYRO].deltatime = div64_s64(slower_deltatime * cdata->fifo_output[ST_MASK_ID_EXT0].sip, cdata->fifo_output[ST_MASK_ID_GYRO].sip);
...
disable_sensor:
	mutex_lock(&sdata->cdata->odr_lock);
	err2 = st_lsm6ds3h_set_enable(sdata, false, true);
	mutex_unlock(&sdata->cdata->odr_lock);
free_buffer_data:
	if (err2 >= 0)
...
reenable_sensor:
	mutex_lock(&sdata->cdata->odr_lock);
	st_lsm6ds3h_set_enable(sdata, true, true);
	mutex_unlock(&sdata->cdata->odr_lock);
