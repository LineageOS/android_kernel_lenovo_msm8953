...
#define ST_LSM6DS3H_GYRO_FS_245_GAIN			IIO_DEGREE_TO_RAD(4375)
#define ST_LSM6DS3H_GYRO_FS_500_GAIN			IIO_DEGREE_TO_RAD(8750)
#define ST_LSM6DS3H_GYRO_FS_1000_GAIN			IIO_DEGREE_TO_RAD(17500)
	...
	if (sdata->cdata->sensors_use_fifo > 0) {
		...
		if (fifo_conf_changed) {
			...
			if ((samples_in_pattern[0] > 0) || (samples_in_pattern[1] > 0) || (samples_in_pattern[2] > 0)) {
				...
				if ((temp_num_samples[0] > 0) && (samples_in_pattern[0] > 0)) {
					unsigned int n_gen;
				...
				if ((temp_num_samples[1] > 0) && (samples_in_pattern[1] > 0)) {
					unsigned int n_gen;
					int64_t temp_deltatime = 0;
