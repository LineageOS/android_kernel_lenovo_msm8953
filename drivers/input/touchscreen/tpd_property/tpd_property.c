		...
		struct kobj_attribute *attr, char *buf)
{
	...
	if (!have_correct_setting)
		return sprintf(buf, "Don't have the right info yet.\n");
	else
		return sprintf(buf, "Product ID=%s; Build ID=%x; "
				"Config ID=0x%08x;\n", tpd_info_t->product_id,
				tpd_info_t->build_id, tpd_info_t->config_id);
}
		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	pr_debug("glove mode = %d, res = %d.\n", mode, res);
	if (res != 1) {
		pr_err("%s: expecting 1 number\n", __FUNCTION__);
		return -EINVAL;
	} else {
		set_glove_ctrl(mode);
	}
