		...
		int current_ma)
{
	...
	rc = smb1351_masked_write(chip, CMD_INPUT_LIMIT_REG, 0x2,
			(current_ma > 100) ? 0x2 : 0x0);
	if (rc) {
...
int smb1351_hv_handshake(struct smb1351_charger *chip)
{
	...
	rc = smb1351_masked_write(chip, CMD_INPUT_LIMIT_REG, BIT(0),
			CMD_USB_HC_MODE);
	if (rc) {
...
static int smb1351_get_board_temp_voltage(struct smb1351_charger *chip,
		enum TEMP_TYPE type)
{
	int rc = 0;
...
static int smb1351_get_board_temp(struct smb1351_charger *chip,
		enum TEMP_TYPE type)
{
	int i;
	const int cnt_num = 6;
...
static int smb1351_parse_dt(struct smb1351_charger *chip)
{
	...
	chip->chg_en_gpio = of_get_named_gpio_flags(node,
					"qcom,en-gpio", 0, 0);
	chip->chg_led_gpio = of_get_named_gpio_flags(node,
					"qcom,led-gpio", 0, 0);
#ifdef LENOVO_OTG_USB_SHORT
	chip->otg_usb_short_gpio = of_get_named_gpio_flags(node,
					"qcom,otg-short-gpio", 0, 0);
#endif
