...
void tusb422_shedule_work(void)
{
	schedule_delayed_work(&tusb422_pd->pal_work, msecs_to_jiffies(2000));
}
#ifdef TUSB422_DEBUG
							 ...
							 const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&tusb422_pd->pal_work, tusb422_pal_otg_work);
#ifdef CONFIG_WAKELOCK
...
static int tusb422_remove(struct i2c_client *client)
{
	...
	hrtimer_cancel(&tusb422_pd->timer);
	cancel_work_sync(&tusb422_pd->work);
	cancel_delayed_work_sync(&tusb422_pd->pal_work);
#ifdef CONFIG_WAKELOCK
	wake_lock_destroy(&tusb422_pd->attach_wakelock);
	wake_lock_destroy(&tusb422_pd->detach_wakelock);
#endif
