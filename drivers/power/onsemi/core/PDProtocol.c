...
void ProtocolIdle(void)
{
    ...
    else if ((Registers.Status.I_GCRCSENT || (!Registers.Status.RX_EMPTY && !Registers.Status.ACTIVITY)) && !ProtocolMsgRx)      // Otherwise check to see if we have received a message and sent a GoodCRC in response
    {       
        if(!g_collision_lock)
        {
            ProtocolFlushTxFIFO();
